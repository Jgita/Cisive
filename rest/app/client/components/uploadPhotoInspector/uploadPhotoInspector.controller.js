app.controller('uploadPhotoInspectorController', [
    '$scope',
    '$http',
    '$q',
    'globalService',
    ...
    function ($scope, $http, $q, globalService, $uibModal, toastr, $httpParamSerializerJQLike, $state, $css, $rootScope, CONSTANTS) {
        ...
        $scope.uploadPhotos = function () {
            ...
            const inspectorPhotoPromis = new Promise(function (resolve, reject) {
                        ...
                        if ($rootScope.photosUploadedByUser.rear != null && $rootScope.photosUploadedByUser.rear != undefined) {
                            $http({
                                ...
                                data: $httpParamSerializerJQLike($rootScope.photosUploadedByUser.rear),
                                    headers: {
                                        'Content-Type': 'application/x-www-form-urlencoded',
                                        'Authorization': 'bearer ' + sessionStorage.accessToken+1212
                                    }
                                })
                                .then(function (rearResp) {
                                    $rootScope.isProcessShow(false);
                                    if (rearResp.data.StatusCode == 200) {
                        ...
                        EPAPromise = new Promise(function (eresolve, ereject) {
                            if ($rootScope.photosUploadedByUser.EPA != null && $rootScope.photosUploadedByUser.EPA != undefined) {
                                $http({
                                    ...
                                    .then(function (EPAResp) {

                                        $rootScope.isProcessShow(false);
                                        if (EPAResp.data.StatusCode == 200) {
            ...
            inspectorPhotoPromis.then(function (response) {
                $rootScope.isProcessShow(false);
